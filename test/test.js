...
function createMarble(color, quantity=1, cost=1, callback) {
  cmd.get(
    'bright console issue command "F CICSTRN1,MB06 CRE ' + color + " " + quantity + " " + cost + '" --cn CUST006',
    function (err, data, stderr) {
...
function deleteMarble(color, callback) {
  cmd.get(
    'bright console issue command "F CICSTRN1,MB06 DEL ' + color + '" --cn CUST006',
    function (err, data, stderr) {
      typeof callback === 'function' && callback(err, data, stderr);
    }
  );
...
function getMarbleQuantity(color, callback) {
  ...
  cmd.get(
    'bright jobs submit data-set "CUST006.MARBLES.JCL(MARBIND)" --rff jobid --rft string',
    function (err, data, stderr) {
      if(err){
        callback(err);
      } else {
...
function updateMarble(color, quantity, callback) {
  cmd.get(
    'bright console issue command "F CICSTRN1,MB06 UPD ' + color + " " + quantity + '" --cn CUST006',
    function (err, data, stderr) {
      typeof callback === 'function' && callback(err, data, stderr);
    }
  );
